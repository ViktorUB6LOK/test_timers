...
// ----------Инициализация модулей AD9833 (вкл-выкл в setting.h)-----------------------------------
void INIT_AD9833 () {
  #ifdef Modul_1_ON
...
// Функция использует данные с dwin, проверка обновления данных не нужна -
// dwin передает только измененные данные по сравнению с предыдущей передачей.
// проверка нужна будет если данные поступают из вне или от генератора ...
// проверка изменений необходима чтоб лишний раз не писать данные в AD9833!!!
// dwin передает каждый раз новые данные в диапазоне 0-200 л/мин (сам ограничивает их)
...
//	if (((data_flowmeter != old_data_flowmeter) || (setting_ratio_flowmeter != old_setting_ratio_flowmeter)) && (flowmeter_freq <= flowmeter_freq_max))
  {
	AD9833_SetWaveData(flowmeter_freq, 0); // передача частоты на модуль, 1- меандр
...
//		if (((data_speedmeter != old_data_speedmeter) || (setting_ratio_speedmeter != old_setting_ratio_speedmeter)) && (speedmeter_freq <= speedmeter_freq_max))
	{
		AD9833_SetWaveData_2(speedmeter_freq, 0); // передача частоты на модуль, 2- меандр
...
void power_off_ad9833 (){
	...
	AD9833_SetWaveData(.0f, 0);  // пока так - далее переделать что откл модуль совсем!
}

void power_off_ad9833_2 (){
	// функция выключения модуля AD9833;
	AD9833_SetWaveData_2(.0f, 0);  // пока так - далее переделать что откл модуль совсем!
}
